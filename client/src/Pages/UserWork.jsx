...
function UserWork() {
  const { shots, loading, shotViews } = useContext(ShotContext);
  ...
  return (
    <>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 p-4 md:p-6 mx-4 md:mx-8 lg:mx-20">
          {shots.map((shot) => (
            <div key={shot._id} className="flex flex-col">
              <Link to={`/shots/${shot._id}`}>
              ...
              {/* Stats Container */}
              <div className="flex items-center justify-end bg-white rounded-lg p-2 md:p-3">
        ...
        <div className="p-4 md:p-8 border-2 w-full max-w-sm md:max-w-md mx-auto md:ml-20 mb-8 md:mb-12 border-dashed border-gray-300 rounded-lg">
          <div className="flex flex-col mt-2 items-center justify-center text-center space-y-4">
            ...
            <Link to="/uploads">
              <button className="px-4 md:px-6 py-2 bg-black text-white rounded-full hover:bg-gray-800 transition-colors text-sm md:text-base">
                Upload your first shot
              </button>
            </Link>
          </div>
